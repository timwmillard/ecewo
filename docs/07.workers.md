...
3. [Notes](#notes)
4. [Usage in Middleware](#usage-in-middleware)
...
> [!CAUTION]
>
> If you want to set a route-specific timeout using `request_timeout()` in an async handler, you must call it before calling `spawn()` or `spawn_http()`. DO NOT call `request_timeout()` in a worker!
...
void auth_middleware(Req *req, Res *res, Next next) {
  ...
  spawn_http(res, ctx, verify_token_work, verify_token_done);
...
void verify_token_done(Res *res, void *context) {
  auth_context_t *ctx = (auth_context_t *)context;
  
  if (ctx->valid) {
    ctx->next(ctx->req, res);
  } else {
    send_text(res, 401, "ERROR: Invalid token");
  }
}
```
